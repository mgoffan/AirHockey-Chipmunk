...
- (void)setUpGameLayer {
    puck1P = [[Puck alloc] initWithPosition:ccp(20, 160) size:CGSizeMake(55, 60)];
    puck2P = [[Puck alloc] initWithPosition:ccp(460, 160) size:CGSizeMake(50, 60)];
//    puck[0].position = ccp(20, 160);
//    puck[1].position = ccp(460, 160);
    puck1P.tag = 0;
    puck2P.tag = 1;
    gameLayer.isTouchEnabled = YES;
    [gameLayer addChild:puck1P z:0];
    [gameLayer addChild:puck2P z:0];
...
- (void)touchDidBegin:(UITouch *)touch withEvent:(UIEvent *)anEvent puck:(NSInteger)thePuck {
    NSLog(@"aaa");
//    CGPoint location = [[[touches allObjects] objectAtIndex:0] locationInView:[[[touches allObjects] objectAtIndex:0] view]];
//    CGPoint location2 = [[[touches allObjects] objectAtIndex:1] locationInView:[[[touches allObjects] objectAtIndex:1] view]];
//    if (location.x > 240) {
...
- (void)touchMoved:(UITouch *)touch withEvent:(UIEvent *)anEvent puck:(NSInteger)thePuck {
    NSLog(@"aaa");
}
